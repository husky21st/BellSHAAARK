...
shandle.setFormatter(
    logging.Formatter(
        ...
        '%(message)s'))
log = logging.getLogger('ALGORIZERO')
log.propagate = False
...
def compute_loss(outputs, targets):
    rescal_targets = torch.sign(targets) * (torch.sqrt(torch.abs(targets) + 1) - 1) + 0.001 * targets
    loss = torch.mean(torch.square(outputs - rescal_targets))
    return loss
    ...
    for device in device_iterator:
        ...
        for player_id in range(len(state_shape)):
            specs = dict(
                ...
                record=dict(size=(T, 20, 67), dtype=torch.int8),
...
def act(
    ...
    try:
        ...
        print(f'lean_model_id is {learn_model_ids}')
        ...
        record_buf = [[] for _ in range(env.num_players)]
            ...
            for p in range(env.num_players):
                ...
                while size[p] > T:
                    ...
                    for t in range(T):
                        ...
                        buffers[p]['record'][index][t, ...] = record_buf[p][t]

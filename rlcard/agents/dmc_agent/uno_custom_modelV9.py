            ...
            if len(hand) == 1:
                ...
                if len(uni_legal_actions) == 1:
                    ...
                    if have_wilds['shuffle_wild']:
                        if min_card_and_skips <= 3 or hand_wild_num <= 1:
                    ...
                    if legal_number_actions or legal_symbol_actions:
                        if not legal_symbol_actions:
                            ...
                            if not legal_number_actions:
                                ...
                                if 'skip' in legal_actions_trait and next_card_num == 1:
                                    action = self.symbol_action('skip', legal_symbol_actions, open_color)
                                    return action
                                if 'draw_2' in legal_actions_trait and next_card_num == 1:
                                    action = self.symbol_action('draw_2', legal_symbol_actions, open_color)
                                    return action
                                if 'reverse' in legal_actions_trait and next_card_num <= 4 and next_card_num < prev_card_num - 2:
                                    action = self.symbol_action('reverse', legal_symbol_actions, open_color)
                                    return action
                                ...
                                if min_action in self.REVERSE and len(legal_symbol_actions) >= 2:
                                    # reverse
                                    if action_point[0][1] <= action_point[1][1] - 8 and prev_card_num != 1:
                                        action = min_action
                        ...
                        # wild only
                        for _wild_action in legal_wild_actions:
                            if have_wilds['wild'] and _wild_action in self.WILD:
                                action = self.action_wild('wild', hand, played_cards)
                                return action
                        for _wild_action in legal_wild_actions:
                            if have_wilds['white_wild'] and _wild_action == self.WILD_OTHERS[0]:
                                action = self.action_wild('white_wild', hand, played_cards)
                                return action
                        for _wild_action in legal_wild_actions:
                            if have_wilds['wild_draw_4'] and _wild_action in self.WILD_DRAW_4:
                                action = self.action_wild('wild_draw_4', hand, played_cards)
                                return action
                        if have_wilds['white_wild']:
                            return 'draw'
